...
#FUNCTION FOR DOOR AND TO SEND DATA TOWARD FIREBASE
def open_door_data() :
...
def scan_attendance_open() :
    if not(GPIO.input(irPin)):
        attendanceStatus = "ATTENDANCE OPENED..."
        ...
        attendanceStatus = "ATTENDANCE CLOSED!!!"
        print(attendanceStatus)
        
def scan_attendance_open_deny() :
    if not(GPIO.input(irPin)):
        attendanceStatus = "ATTENDANCE OPENED..."
        print(attendanceStatus)
...
def scan_attendance_close() :
    if not(GPIO.input(irPin)):
        attendanceStatus = "ATTENDANCE CLOSED BECAUSE NOT MEET REQUIREMENTS!!!"
        print(attendanceStatus)
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if mask > withoutMask:
            ...
            if bodyTemp < 37:
                scan_attendance_open()
            else :
                scan_attendance_open_deny()
                GPIO.output(buzzerPin, GPIO.HIGH)
                time.sleep(1)
                GPIO.output(buzzerPin, GPIO.LOW)
                time.sleep(1)
